package Monitoring::Zabipi::SetEnv;
use strict;
use warnings;
...
sub zbx_setenv {
  ...
  open (my $fhSetEnv, '<', $pthSetEnvFile) 
    or die sprintf('Cant get environment from file %s: %s', $pthSetEnvFile, $!);
  my %zbxEnv = 
    map { 
      chomp;
      m/^\s*(?<KEY>[A-Za-z0-9_-]+)\s*=\s*(?:(?<Q>["'])(?<VAL>((?!\g{Q}).)*)\g{Q}|(?<VAL>[^'"[:space:]]+?))\s*$/
        ? ($+{'KEY'}, $+{'VAL'})
        : qw[NOTHING NOWHERE]
    }
      grep !/^\s*(?:#.*)?$/, <$fhSetEnv>;
